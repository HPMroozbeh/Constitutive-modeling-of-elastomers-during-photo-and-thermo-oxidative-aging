...
F_s=[Lambda_s^(-2),0,0;0,Lambda_s,0;0,0,Lambda_s];
...
% is 1 day aging (except for 45 degree which is 10 days). 4th is 10 days
% (except for 45 degree which is 60 days) 
% and 5th is 30 days (except for 45 degree which is 150 days). Therefore,
% just change the "?" in num(:,?), to calculate the respecting stress with
% respect to time. For instance if t is t = 10*24*3600 the stress is num(:,4).
% Experimental data point
figure(1);
hold on
 num = xlsread('Thermo_60.xlsx');
 strain_exp=num(:,5);
 stress_exp=num(:,6);
or = [0.93 0.69 0.13];
pu = [0.71 0.27 1];
gr = [ 0.39 0.83 0.07];
plot(-Macro_strain,stress,'-','Color',pu,'LineWidth',3)
 hold on
 plot(strain_exp,stress_exp,'p','Color','k','LineWidth',3,'MarkerSize',9)
xlim([-1 5])
%ylim([0 10])
 title('\theta = 60')
 xlabel('Strain ') 
