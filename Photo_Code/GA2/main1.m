...
% importing the data
Aged_data = xlsread('BPU_45150.xlsx');
% Aged_data1 = xlsread('BPU_4560.xlsx');
Aged_data2 = xlsread('BPU_8010.xlsx');
% Aged_data3 = xlsread('BPU_6030.xlsx');
Aged_data4 = xlsread('BPU_8030.xlsx');
Aged_data5 = xlsread('BPU_6010.xlsx');
...
Aged_stretch5 = Aged_data5(:,1)+1;
Aged_stress5 = Aged_data5(:,2);
...
for i=1:ngen
   ...
   for i1=1:pop_size
       yy(i1) = func1(xx(i1,:),Aged_stretch,Aged_stress,Aged_stretch2,Aged_stress2, Aged_stretch4,Aged_stress4,Aged_stretch5,Aged_stress5);
   end
...
uuu = Aging(x_min(end,:),150*24*3600,318,Aged_stretch);
...
% uuu1 = Aging(x_min(end,:),60*24*3600,318,Aged_stretch1);
...
uuu2 = Aging(x_min(end,:),10*24*3600,353,Aged_stretch2);
...
% uuu3 = Aging(x_min(end,:),30*24*3600,333,Aged_stretch3);
% x4 = linspace(1,(length(Aged_stretch3)-1)/4+1,nos);
% plot(x4,uuu3(1,:),'-','Color','c','LineWidth',3)
plot(Aged_stretch4,Aged_stress4,'s','Color','g','LineWidth',3,'MarkerSize',9)
uuu4 = Aging(x_min(end,:),30*24*3600,353,Aged_stretch4);
x5 = linspace(1,(length(Aged_stretch4)-1)/4+1,nos);
plot(x5,uuu4(1,:),'-','Color','g','LineWidth',3)
plot(Aged_stretch5,Aged_stress5,'p','Color','c','LineWidth',3,'MarkerSize',9)
uuu3 = Aging(x_min(end,:),10*24*3600,333,Aged_stretch5);
x4 = linspace(1,(length(Aged_stretch5)-1)/4+1,nos);
plot(x4,uuu3(1,:),'-','Color','c','LineWidth',3)
