...
R_2 = 0.0344;
R_I = xx(6);
Rbar_b = (Rbar_s) - (R_1 + (R_I/2))*(1-exp(-exp(-E2/(R*temp))*I^alpha*(t/T2)))+ (R_2 - (R_I/2))*(1-exp(-exp(-E2/(R*temp))*I^beta*(t/T2)));
% an indicator for crosslink density
q_s = 1.0;
q_1 = 0.0441;
q_2 = 0.01877;
q_I = xx(7);
q_b = (q_s) - (q_1 + (q_I/2))*(1-exp(-exp(-E2/(R*temp))*I^alpha*(t/T2)))+ (q_2 - (q_I/2))*(1-exp(-exp(-E2/(R*temp))*I^beta*(t/T2)));
